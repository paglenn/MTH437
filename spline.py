...
# Homework 7 Section 3.4 Exercise 8b
...
def splineplot(x,y):
	...
	for i in range(n-1):
		...
		plot(x0, coeff[i,0]+2*coeff[i,1]*(x0-x[i])+3*coeff[i,2]*(x0-x[i])**2.,'r')
		plot(x0,2*coeff[i,1]+6*coeff[i,2]*(x0-x[i]),'b')
		plot(x0,6*coeff[i,2]*ones(size(x0)),'k')
		ylim(-8,8)
#		plot([x[i],x[i+1]],[y[i],y[i+1]],'bo')
		plot(x0,y0)
		ylabel('y')
		xlabel('x')
		title('Spline plot with derivatives')
		savefig('ex8bderivatives.png')
	show()
